...
<div class="mb-3">
	...
	<input id="title" class="form-control" type="text" name="title" value="{{ $inventory->title ?? old( 'title' ) }}" autofocus="autofocus"/>
	<div class="form-text">{{ __( 'например: чайник' ) }}</div>
</div>
<div class="mb-3">
	<label for="price">{{ __( 'Цена' ) }}</label>
	<input id="price" class="form-control" type="text" name="price" value="{{ $inventory->currentPrice ? $inventory->currentPrice->price : old( 'price' ) }}"/>
